		...
		//	global $hooks_menu;
			...
			foreach ( $this->plugins as $plugin ) {
				if ( ! is_plugin_active( $plugin['file_path'] ) ) {
					pagelines_insert_menu(
						$this->strings['page_title'],				// Parent menu slug
	...
	//	$hooks_menu = pagelines_insert_menu( PL_MAIN_DASH, 'Hook Editor', 'edit_theme_options', 'pagelines_hooks', array( &$this, 'get_admin_page' ) );
		...
		protected function do_plugin_install() {
			...
			if ( isset( $_GET[sanitize_key( 'tgmpa-install' )] ) && 'install-plugin' == $_GET[sanitize_key( 'tgmpa-install' )] ) {
				...
				$slug   = $_GET[sanitize_key( 'slug' )]; // Plugin slug

				foreach( $this->plugins as $k => $d ) {
				...
				if( 'theme' == $plugin['pl_type'] )
					$upgrader = new Theme_Upgrader( $skin = new Theme_Installer_Skin( compact( 'type', 'title', 'url', 'nonce', 'plugin', 'api' ) ) );
				else
				...
				if ( is_wp_error( $upgrader->skin->result ) ) {
					$error_string = $upgrader->skin->result->get_error_message();
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		function single_row( $item ) {
			...
			$extra = ( isset( $_GET['pl_installed'] ) && $_GET['pl_installed'] == $item['slug'] ) ? 'new' : '';

			$class = sprintf( ' class="%s"', $extra );
		...
		protected function _gather_plugin_data() {
			...
			foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
				if ( 'active' == $this->get_ext_state( $plugin ) )
				...
				elseif ( ! $source || preg_match( '|http://www.wordpress.org|', $source ) ) {
					$url = add_query_arg(
						array(
							'tab'       => 'plugin-information',
							'plugin'    => $this->get_plugin_data( $plugin ),
							'TB_iframe' => 'true',
							'width'     => '640',
							'height'    => '500',
						),
				...
				$state = $this->get_ext_state( $plugin );
				...
				$table_data[$i]['pl_type'] = $plugin['pl_type'];
...
return;
		...
		// Return state of extension
		// active
		// installed
		// false
		function get_ext_state( $item ) {

			if( 'theme' == $item['pl_type'] )
				return $this->theme_status( $item );

			if( 'plugin' == $item['pl_type'] )
				return $this->plugin_status( $item );
		}


		function plugin_status( $item ) {
		...
		public function column_plugin( $item ) {
			...
			elseif ( 'installed' == $state ) {
				...
				// if its a theme.. redirect to themes page...
				if( 'theme' == $item['pl_type'] ) {
					$url = sprintf( '<a href="%1$s" title="Activate %2$s">Activate</a>', admin_url( 'themes.php'), $item['sanitized_plugin'] );
				} else {
					$url = sprintf(
		...
		public function get_columns() {
			...
			$columns = array(
				...
				'desc' => __( 'Description', TGM_Plugin_Activation::$instance->domain ),
				'version'   => __( 'Version', TGM_Plugin_Activation::$instance->domain ),
...
class OverLoad_Theme extends Theme_Upgrader {
